...
const imageChoser = (author) => {
  if (author.length >= 10) {
    return 'https://kitt.lewagon.com/placeholder/users/papillard'
  } else if (author.length >= 5) {
    return 'https://kitt.lewagon.com/placeholder/users/pdunleav'
  } else {
    return 'https://kitt.lewagon.com/placeholder/users/arthur-littm'
  }
}
...
const listItemBuilder = (content, time, author) => {
  ...
  const imageMe = 'https://avatars.githubusercontent.com/u/94529648?v=4';
  const imageOthers1 = '';
  const date = new Date();
  ...
  const message = `<div class="chat-message-container ${minutesAgo === 0 ? 'new-message' : ''}">
                    <img src="${author === 'Patrick' ? imageMe : imageChoser(author)}" alt="">
